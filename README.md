...
- Fortschrittsansicht: Der Button `Fortschritt anzeigen` zeigt den Gesamtstand und fuer jede Rubrik getrennte Balken fuer `Basis`, `Training` und `Plus`. Ein Klick auf eine Rubrik fuehrt zu ihren Basisaufgaben.
...
Bei einem bereits veroeffentlichten Repository fuer dieses Update `index.html`, `styles.css` und `app.js` ersetzen. GitHub Pages veroeffentlicht die Aenderung danach erneut. Der Fortschritt bleibt beim normalen Aktualisieren im selben Browser erhalten.
